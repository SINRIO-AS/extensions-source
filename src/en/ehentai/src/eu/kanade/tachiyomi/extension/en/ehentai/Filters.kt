...
internal class PageOrderFilter : Filter.Select<String>(
    "Order within each loaded page",
    arrayOf("Website order", "Most pages first", "Fewest pages first", "Title A-Z", "Title Z-A"),
)
...
internal class ExactTagFilter : Filter.Select<String>(
    "Tag matching",
    arrayOf("Prefix / normal matching", "Exact tags only"),
)
...
internal class ArtistTagsFilter : Filter.Text("Artist tags (comma separated)")

internal class GroupTagsFilter : Filter.Text("Group / circle tags (comma separated)")

internal class ParodyTagsFilter : Filter.Text("Parody / series tags (comma separated)")

internal class CharacterTagsFilter : Filter.Text("Character tags (comma separated)")

internal class FemaleTagsFilter : Filter.Text("Female tags (comma separated)")

internal class MaleTagsFilter : Filter.Text("Male tags (comma separated)")

internal class CosplayerTagsFilter : Filter.Text("Cosplayer tags (comma separated)")

internal class LocationTagsFilter : Filter.Text("Location tags (comma separated)")

internal class OtherTagsFilter : Filter.Text("Other namespace tags (comma separated)")

internal class UploaderFilter : Filter.Text("Uploader (comma separated)")

internal class GalleryIdFilter : Filter.Text("Gallery ID (comma separated)")

internal class TitleQueryFilter : Filter.Text("Title-only terms (comma separated)")
...
internal fun String.searchTerms(
    exclude: Boolean = false,
    exact: Boolean = false,
    namespace: String? = null,
): List<String> = split(',').map { it.trim() }.filter { it.isNotEmpty() }.map { term ->
    val qualified = if (namespace != null && ':' !in term) "$namespace:$term" else term
    val exactTerm = if (exact && !qualified.endsWith('\$') && ' ' !in qualified) {
        "${qualified}${'\$'}"
    } else {
        qualified
    }
    if (exclude) "-$exactTerm" else exactTerm
}
